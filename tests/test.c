...
void p_expect(const Parser *p, const TokenType expected_type) {
    if (!p_is_last_token(p)) {
        ...
        if (!(token->type == expected_type)) do { log_message(LOG_ERROR, "[%d:%d]: Expected %tk got %tk\n", token->line_n, token->char_n, expected_type, token->type); exit(1); } while (0);
    }
}
...
Node *p_parse_init_list(Parser *p, NodeManager *nm) {
    ...
    while (p_peek(p)->type != TK_CLOSE_CURLY) {
        if (p_peek(p)->type == TK_DOT) {
            ...
            member_assign->designator.kind = T_STRUCT;
            member_assign->designator._struct.name = token->value;
            member_assign->designator.value = p_parse_expression(p, nm, 0);
            ...
            element_assign->designator.value = p_parse_expression(p, nm, 0);
